...
;(function ( $, window, document, undefined ) {
	...
	var pluginName = "growl",
		...
		defaults = {
			...
			onHidden: null,
			icon_type: 'class',
			template: '<div data-growl="container" class="alert" role="alert"><button type="button" aria-hidden="true" class="close" data-growl="dismiss">&times;</button><span data-growl="icons"></span><span data-growl="title"></span><span data-growl="message"></span><a href="#" data-growl="url"></a></div>'
		};
	...
	addContent = function($template, settings) {
		...
		if (settings.content.icon) {
			if (settings.icon_type.toLowerCase() == 'class') {
				$template.find('[data-growl="icons"]').addClass(settings.content.icon);
			}else{
				if ($template.find('[data-growl="icons"]').is('img')) {
					$template.find('[data-growl="icons"]').attr('src', settings.content.icon);
				}else{
					$template.find('[data-growl="icons"]').append('<img src="'+settings.content.icon+'" />');
				}
			}
		}
			...
			switch (command) {
				case 'icon':
					if (this.settings.icon_type.toLowerCase() == 'class') {
						this.$template.find('[data-growl="icons"]').removeClass(this.settings.content.icon);
						this.$template.find('[data-growl="icons"]').addClass(update);
					}else{
						if (this.$template.find('[data-growl="icons"]').is('img')) {
							this.$template.find('[data-growl="icons"]')
						}else{
							this.$template.find('[data-growl="icons"]').find('img').attr().attr('src', update);
						}
					}
					break;
				case 'url':
